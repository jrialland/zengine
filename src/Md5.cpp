...
void Md5Digest::update(const void *data, size_t len)
{
    md5Update((MD5Context *)context, (uint8_t *)data, len);
}

void Md5Digest::digest(uint8_t hash[16])
{
    md5Finalize((MD5Context *)context);
    memcpy(hash, ((MD5Context *)context)->digest, 16);
}

std::string Md5Digest::hexdigest()
{
    uint8_t hash[16];
    digest(hash);
    std::stringstream ss;
...
std::string Md5Digest::hexdigest(const void *data, size_t len)
{
    Md5Digest digest;
...
std::string Md5Digest::hexdigest(const std::string &data)
{
    return hexdigest(data.data(), data.size());
}

std::string Md5Digest::hexdigest(std::istream &stream)
{
    Md5Digest digest;
    char buffer[4096];
    while (!stream.eof())
    {
        ...
        digest.update(buffer, stream.gcount());
    }
    return digest.hexdigest();
}